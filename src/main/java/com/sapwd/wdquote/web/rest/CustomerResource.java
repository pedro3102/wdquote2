...
/**
 * REST controller for managing {@link com.sapwd.wdquote.domain.Customer}.
 */
@RestController
@RequestMapping("/api")
public class CustomerResource {
    ...
    /**
     * {@code GET  /customer} : get all the customers with basic info.
     *
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of customers in body.
     */
    @GetMapping("/customers/basic")
    public ResponseEntity<List<CustomerDTO>> getAllCustomersBasic() {
        log.debug("REST request to get a list of Customers basic");
        List<CustomerDTO> customerDTOList = customerService.findAllBasic();
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createAlert(applicationName, "List of Customers ", ENTITY_NAME))
