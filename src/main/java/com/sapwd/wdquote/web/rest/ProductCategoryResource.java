...
/**
 * REST controller for managing {@link com.sapwd.wdquote.domain.ProductCategory}.
 */
@RestController
@RequestMapping("/api")
public class ProductCategoryResource {
    ...
    /**
     * {@code GET  /product-categories/basic} : get all the product categories with basic info.
     *
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of product categories in body.
     */
    @GetMapping("/product-categories/basic")
    public ResponseEntity<List<ProductCategoryDTO>> getAllProductCategoriesBasic() {
        log.debug("REST request to get a list of product categories with a basic info");
        List<ProductCategoryDTO> productCategoryDTOList = productCategoryService.findAllBasic();
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createAlert(applicationName, "List of product categories ", ENTITY_NAME))
            .body(productCategoryDTOList);
    }
}
