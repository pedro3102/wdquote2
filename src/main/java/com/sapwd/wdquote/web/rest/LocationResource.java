...
/**
 * REST controller for managing {@link com.sapwd.wdquote.domain.Location}.
 */
@RestController
@RequestMapping("/api")
public class LocationResource {
    ...
    /**
     * {@code GET  /locations} : get all the locations basic.
     *
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of locations in body.
     */
    @GetMapping("/locations/basic")
    public ResponseEntity<List<LocationDTO>> getAllLocationsBasic() {
        log.debug("REST request to get a list of Locations basic");
        List<LocationDTO> locationDTOList = locationService.findAllBasic();
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createAlert(applicationName, "List of Locations Basic", ENTITY_NAME))
