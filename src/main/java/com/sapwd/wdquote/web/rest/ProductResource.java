...
import com.sapwd.wdquote.service.ProductQueryService;
...
/**
 * REST controller for managing {@link com.sapwd.wdquote.domain.Product}.
 */
@RestController
@RequestMapping("/api")
public class ProductResource {
    ...
    /**
     * {@code GET  /products} : get all the products basic.
     *
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of products basic in body.
     */
//    @GetMapping("/products/basic")
//    public ResponseEntity<List<ProductDTO>> getAllProductsBasic() {
//        log.debug("REST request to get a list of Products basic");
//        List<ProductDTO> productDTOList = productService.findAllBasic();
//        return ResponseEntity
//            .ok()
//            .headers(HeaderUtil.createAlert(applicationName, "List of Products Basic", ENTITY_NAME))
//            .body(productDTOList);
//    }
    @GetMapping("/products/basic")
    public ResponseEntity<List<ProductDTO>> getAllProductsBasic(
        ...
        log.debug("REST request to get Products Basic by criteria: {}", criteria);
        Page<ProductDTO> page = productQueryService.findByCriteriaBasic(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}
